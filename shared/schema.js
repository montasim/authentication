 ...
 * The shared schemas encompass a wide range of (users) attributes, from basic personal information like usernames and dates of birth to more complex data like social media profiles and security settings.
 ...
 * Captures the (users)'s date of birth, required for age-based access control. It ensures the date is in the specified format (DD-MM-YYYY) and is a valid date.
 */
const dateOfBirthSchema = {
    ...
    description:
        "The (users)'s birth date, important for age-based access control.",
};

/**
 * @schema bioSchema
 * Stores a brief (users) biography. This field is trimmed for whitespace and has a maximum character length limit to ensure concise (users) descriptions.
 */
const bioSchema = {
    type: String,
    trim: true,
    maxlength: [
        usersConstants.lengths.BIO_MAX,
        `Bio should not exceed ${usersConstants.lengths.BIO_MAX} characters.`,
    ],
    description:
        'A short description about the (users), limited to a specified maximum length.',
};

/**
 * @schema pronounsSchema
 * Links to an external Pronouns model, allowing the (users) to select pronouns from a predefined list. This schema helps in promoting inclusivity within the application.
 */
 ...
 * Represents a unique numeric identifier for a (users), which is sparse and unique. It is used for systems that require a numeric ID alongside traditional string-based identifiers.
 */
const numericIdSchema = {
    ...
    // required: [true, 'A numeric ID is required for each (users).'],
    description:
...
/**
 * @schema userIdSchema
 * Stores a unique (users) ID in a predefined format, ensuring uniqueness across the system. This ID is crucial for linking (users) activities and data securely.
 */
const userIdSchema = {
    type: String,
    trim: true,
    ...
    // required: [
    //     true,
    //     'A (users) ID is required and must be unique across all (users).',
    // ],
    unique: [
        true,
        'The (users) ID must be unique. Duplicate (users) IDs are not allowed.',
    ],
    description:
        'User ID in the format (users)-X, where X is an auto-incremented number.',
};
...
const imageSchema = new Schema({
    ...
    shareableLink: {
        ...
        description:
            "URL link that allows others to access the (users)'s image.",
    },
    downloadLink: {
        type: String,
        trim: true,
        ...
        description: "URL link to directly download the (users)'s image.",
    },
});

/**
 * @schema nameSchema
 * Handles storage of (users) names, broken down into first, middle, last, and nick names, each with specific formatting and length requirements to maintain data consistency.
 */
const nameSchema = new Schema({
    first: {
        ...
        minlength: [
            ...
            `Name should be at least ${usersConstants.lengths.NAME.FIRST_MIN} characters.`,
        ],
        maxlength: [
            usersConstants.lengths.NAME.FIRST_MAX,
            `Name should not exceed ${usersConstants.lengths.NAME.FIRST_MAX} characters.`,
        ],
        description:
    ...
    middle: {
        ...
        minlength: [
            usersConstants.lengths.NAME.MIDDLE_MIN,
            `Name should be at least ${usersConstants.lengths.NAME.MIDDLE_MIN} characters.`,
        ],
        maxlength: [
            usersConstants.lengths.NAME.MIDDLE_MAX,
            `Name should not exceed ${usersConstants.lengths.NAME.MIDDLE_MAX} characters.`,
        ],
        description:
    ...
    last: {
        ...
        minlength: [
            usersConstants.lengths.NAME.LAST_MIN,
            `Name should be at least ${usersConstants.lengths.NAME.LAST_MIN} characters.`,
        ],
        maxlength: [
            usersConstants.lengths.NAME.LAST_MAX,
            `Name should not exceed ${usersConstants.lengths.NAME.LAST_MAX} characters.`,
        ],
        description:
    ...
    nick: {
        ...
        minlength: [
            usersConstants.lengths.NAME.NICK_MIN,
            `Name should be at least ${usersConstants.lengths.NAME.NICK_MIN} characters.`,
        ],
        maxlength: [
            usersConstants.lengths.NAME.NICK_MAX,
            `Name should not exceed ${usersConstants.lengths.NAME.NICK_MAX} characters.`,
        ],
        description:
 ...
 * Captures (users) address information, including country, state, and city, with each field stored as a trimmed string to ensure accurate geographic data capture.
 */
const addressSchema = new Schema({
    country: {
        ...
        trim: true,
        description:
            'The country where the (users) resides. This field stores the name of the country as a string.',
    },
    state: {
        type: String,
        trim: true,
        description:
            'The state or region within the country where the (users) is located. This field is used to store the state or regional name.',
    },
    city: {
        type: String,
        trim: true,
        description:
            'The city or locality within the state where the (users) lives. This field captures the city name.',
    },
});

/**
 * @schema passwordHashSchema
 * Stores a hashed version of the (users)'s password, necessary for secure authentication and ensuring data privacy.
 */
...
/**
 * @schema roleSchema
 * References a RoleModel to apply role-based access control (RBAC) mechanisms, which restrict access based on the (users)'s assigned role.
 */
const roleSchema = {
    type: Schema.Types.ObjectId,
    ref: 'Roles',
    default: undefined,
    description:
        "Defines the (users)'s role within the application by referencing the RoleModel, restricting access based on the role defined in the RoleModel.",
};

/**
 * @schema twoFactorEnabledSchema
 * Indicates whether two-factor authentication is enabled for the (users), providing an additional layer of security.
 */
 ...
 * Flags whether the (users) is required to change their password at the next login, often used after administrative resets or potential security breaches.
 */
const mustChangePasswordSchema = {
    ...
    default: false,
    description:
        'Flag to indicate if the (users) must change their password at next login for security reasons.',
};

/**
 * @schema resetPasswordVerifyTokenSchema
 * Holds a token used to verify the (users)'s identity during the password reset process, helping secure the process against unauthorized access.
 */
...
const urlSchema = {
    ...
    description:
        'A unique URL associated with the (users), typically for a personal or professional website.',
};

/**
 * @schema emailSchema
 * Validates email addresses to ensure they are unique and properly formatted, crucial for (users) identification and communication.
 */
const emailSchema = {
    type: String,
    trim: true,
 ...
 * Indicates if the specified email is the primary email for the (users), ensuring that only one email is marked as primary.
 */
const isPrimaryEmailSchema = {
    type: Boolean,
    default: false,
    description:
        'Indicates if this is the primary email for the (users). Only one email should be set as primary at any time.',
};

/**
 * @schema isEmailVerifiedSchema
 * Checks if the (users)'s email has been verified, an important step in confirming the authenticity of the (users)'s contact information.
 */
const isEmailVerifiedSchema = {
    type: Boolean,
    default: false,
    description:
        "Flag to indicate whether the (users)'s email has been verified.",
};
...
/**
 * @schema mobileSchema
 * Validates mobile phone numbers, ensuring they are unique and properly formatted. This is important for systems that use mobile numbers as an alternative form of (users) identification.
 */
const mobileSchema = {
    type: String,
    trim: true,
 ...
 * Identifies if the stored mobile number is the primary contact number for the (users), used in systems that handle multiple contact numbers.
 */
const isPrimaryMobileSchema = {
    type: Boolean,
    default: false,
    description:
        'Indicates if this is the primary mobile for the (users). Only one mobile should be set as primary at any time.',
};

/**
 * @schema isMobileVerifiedSchema
 * Indicates whether the (users)'s mobile number has been verified, an important factor for systems relying on SMS or phone calls for authentication or notifications.
 */
const isMobileVerifiedSchema = {
    type: Boolean,
    default: false,
    description:
        "Flag to indicate whether the (users)'s mobile number has been verified.",
};

/**
 * @schema mobileVerifyTokenSchema
 * Stores a token used to verify the mobile number, essential for confirming the (users)'s control over the listed phone number.
 */
 ...
 * Flags whether the (users)'s profile or account is active or has been deactivated, allowing for easy management of (users) status within the system.
 */
 ...
 * Stores and validates a Facebook profile URL, ensuring it is unique and correctly formatted. This is used for linking a (users)'s social media profile.
 */
...
const linkedInSchema = {
    ...
    description:
        'LinkedIn URLs are unique to each (users), adhering to the required LinkedIn format.',
};

/**
 * @schema githubSchema
 * Manages GitHub usernames, validating uniqueness and format to integrate coding community profiles for (users) accounts.
 */
const githubSchema = {
    type: String,
    trim: true,
 ...
 * Stores information about a (users)'s employer, including name and contact details, providing context for professional relationships within the application.
 */
const companySchema = new Schema({
    // TODO: suggest company name when a (users) try to add company
    name: {
        type: String,
        trim: true,
        maxlength: [
            usersConstants.lengths.COMPANY.NAME_MAX,
            `Company name should not exceed ${usersConstants.lengths.COMPANY.NAME_MAX} characters.`,
        ],
        description:
            "Optional field for storing the name of the (users)'s employer, with validation on length.",
    },
...
const externalOAuthSchema = new Schema({
    googleId: {
        ...
        description:
            "Google ID is used for identifying (users)'s Google account for OAuth authentication purposes. Must be unique.",
    },
    facebookId: {
        type: String,
        trim: true,
        ...
        description:
            "Facebook ID is used for identifying (users)'s Facebook account in OAuth integrations. Ensures uniqueness in the system.",
    },
    twitterId: {
        type: String,
        trim: true,
        ...
        description:
            "Twitter ID is used for OAuth authentication and to link the (users)'s Twitter profile to their account on this system. It must be unique.",
    },
    linkedInId: {
        type: String,
        trim: true,
        ...
        description:
            "LinkedIn ID is uniquely associated with the (users)'s LinkedIn account for authentication and profile linkage purposes.",
    },
    githubId: {
        type: String,
        trim: true,
        ...
        description:
            "GitHub ID is a unique identifier used to link and authenticate the (users)'s GitHub profile within our application.",
    },
});

/**
 * @schema loginSchema
 * Tracks details of login attempts, both failed and successful, providing insights into (users) behavior and potential security incidents.
 */
 ...
 * Manages session information, including tokens and their expiration times, crucial for maintaining secure and valid (users) sessions within the application.
 */
const sessionsSchema = new Schema({
    token: {
        ...
        required: [
            true,
            'A session token is required. This token is crucial for authenticating (users) sessions and ensuring secure interaction with the application.',
        ],
        description:
            'A unique session token that is used to authenticate (users) sessions. This token is generated upon successful login and is necessary to access secure parts of the application.',
    },
    expiresAt: {
        type: Date,
        trim: true,
 ...
 * Stores (users) preferences for application themes, allowing for a customized look and feel of the (users) interface based on individual preferences.
 */
const appearanceSchema = new Schema({
    theme: {
        name: {
            ...
            trim: true,
            description:
                'The name of the theme chosen by the (users). This setting determines the overall look and feel of the application, enabling a personalized (users) experience.',
        },
    },
});
 ...
 * Controls the visibility of (users) profiles to others within the application, allowing (users) to manage privacy settings according to their preferences.
 */
const profileVisibilitySchema = {
    ...
    default: 'public',
    description:
        'Controls the visibility of the (users) profile to other (users).',
};

/**
 * @schema createdByAdminSchema
 * References the admins who created the record, used for tracking which administrator has made changes to (users) data or profiles.
 */
const createdByAdminSchema = {
    type: Schema.Types.ObjectId,
    ref: 'Admins',
    description:
        'Reference to the admins who created this record, used for tracking record ownership.',
};

/**
 * @schema updatedByAdminSchema
 * Tracks the last admins to update the record, ensuring accountability and providing a trace of administrative actions on (users) data.
 */
const updatedByAdminSchema = {
    type: Schema.Types.ObjectId,
    ref: 'Admins',
    description:
        'Reference to the admins who last updated this record, used for tracking changes and record ownership.',
};
