...
export default function Constants() {
    ...
    const fetchApiData = async () => {
        try {
            const data = await getData('/api/v1/dashboard/account-status');
    ...
    const handleDeleteClick = async (id) => {
        const deletePromise = deleteData(
            '/api/v1/dashboard/account-status/',
            id
        );
    ...
    const handleSaveClick = async (id) => {
        ...
        const savePromise = updateData(
            `/api/v1/dashboard/account-status/${id}`,
            {
                value: newValue,
                name: newName,
        ...
        <div>
            {data ? (
                <Table>
                    <TableCaption>
                        {data.length
                            ? 'A list of your used account status.'
                            : 'No account status data found.'}
                    </TableCaption>
                    <TableHeader>
                        <TableRow>
